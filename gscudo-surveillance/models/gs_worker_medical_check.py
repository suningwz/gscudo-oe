...
class WorkerMedicalCheck(models.Model):
    ...
    contract_partner_id = fields.Many2one(
        related="gs_worker_id.contract_partner_id", string="Partner_id"
    )
    contract_partner_name = fields.Char(
        compute="_compute_worker_data", string="Azienda", store=True
    )

    company_doctor = fields.Char(
        compute="_compute_worker_data", string="Medico competente", store=True
    )
    medical_locum = fields.Char(string="Medico sostituto")
    medical_supplier = fields.Char(
        related="contract_partner_id.medical_supplier", string="Fornitore assegnato"
    )
    ...
    birth_date = fields.Date(
        related="gs_worker_id.birth_date", string="Data di nascita"
    )
    birth_place = fields.Char(
        related="gs_worker_id.birth_place", string="Luogo di nascita"
    )
    ...
    def _onchange_checktype_id(self):
        for record in self:
            record.gs_medical_check_frequency_id = (
                record.gs_medical_check_type_id.gs_medical_check_frequency_id
            )
    ...
    def _compute_expiry_date(self):
        for record in self:
            if (
                record.execution_date is not False
                and record.gs_medical_check_frequency_id is not False
            ):
    ...
    def _compute_name(self):
        for record in self:
            ...
            record.name = "{} {} {} ".format(
                ...
                if record.execution_date is not False
                else "",
            )
...
class GSWorker(models.Model):
    ...
    gs_worker_medical_check_ids = fields.One2many(
        comodel_name="gs_worker_medical_check",
        inverse_name="gs_worker_id",
        string="Visite/ Analisi",
