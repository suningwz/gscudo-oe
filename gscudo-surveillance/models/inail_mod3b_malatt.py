...
class InailMod3BMalatt(models.Model):
    ...
    inail_malprof_id = fields.Many2one(
        comodel_name="inail_malprof", string="Malattie Professione"
    )
    ...
    def _compute_name(self):
        for record in self:
            record.name = (
                record.inail_mod3b_id.name + " " + record.inail_malprof_id.name
            )
...
class InailMod3B(models.Model):
    ...
    inail_mod3b_malatt_ids = fields.One2many(
        comodel_name="inail_mod3b_malatt",
        inverse_name="inail_mod3b_id",
        string="Malattie",
    )
