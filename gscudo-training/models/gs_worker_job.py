...
class GSWorkerJobType(models.Model):
    ...
    def _onchange_gs_worker_job_type_id(self):
        for record in self:
            for tc in record.gs_worker_job_type_id.gs_certificate_type_ids:
                record.gs_certificate_type_ids = [(4, tc.id)]
            for (
                tc
            ) in (
                record.gs_worker_job_type_id.gs_worker_job_type.gs_certificate_type_ids
            ):
                record.gs_certificate_type_ids = [(4, tc.id)]
