...
class CallScheduler(models.TransientModel):
    ...
    date_to = fields.Date(string='To',default=fields.Date.context_today,required=True)
    call_per_day = fields.Integer(string='chiamate al giorno', default=10,required=True)
    res_model_id =fields.Integer(default = lambda self: self.env["ir.model"].search([('model','=','crm.lead')]).id)
    activity_type_id = fields.Many2one(comodel_name='mail.activity.type', string='Tipo attività')
    summary = fields.Char(string='Summary')

    def _get_model_id(self):
        id = self.env["ir.model"].search([('model','=','crm.lead')])
        return id
    ...
    def schedule_call(self):
        ...
        for lead in leads:
            ...
            lead.message_subscribe([lead.tmk_user_id.id or lead.user_id.id or self.env.user.id],None)
            activity_data = {
                'activity_type_id' : 2,            
                'res_model' : 'crm.lead',
