...
class ProductTemplate(models.Model):
    ...
    product_family_id = fields.Many2one(
        comodel_name="gs_product_family",
        string="Famiglia",
        index=True,
    )
    law_reference = fields.Char(string="Riferimento normativo")

    # template for generating project / task on sale event
    # project_template_id = fields.Many2one(
    #     string="Modello Progetto",
    #     comodel_name="project.project",
    # )
    task_template_id = fields.Many2one(
        string="Modello Attività",
        comodel_name="project.task",
    )

    task_template_planned_hours = fields.Float(
        string="Ore previste", related="task_template_id.planned_hours"
    )
